    ...
    if($action == "editar"){
        ...
        if($cont){
            ...
            $stmt->bindParam( 4 , $tipo, PDO::PARAM_STR | PDO::PARAM_INPUT_OUTPUT, 4000);
            $stmt->bindParam( 5 , $id, PDO::PARAM_STR | PDO::PARAM_INPUT_OUTPUT, 4000);
            $stmt->execute();
            $count = $stmt->rowCount();
            if($count > 0){
                $json['status'] = true;
    ...
    if($action == "agregar"){
        ...
        if($cont){
            ...
            if($count > 0){
                ...
                $json['status'] = false;
                $json['msg'] = "Error al ingresar el trabajador";
            }
        }
        echo json_encode($json);
    }
    ...
    if($action == "buscar"){
        $texto = $_POST["texto"];
            ...
            $db = $modelo->get_conexion();
            if($db){
                $stmt = $db->prepare( "CALL Buscar_Trabajador (?)");
